...
const GrowTraffic = () => {
  ...
  return (
    <div className="relative">
      <div className="mt-4 md:-mt-[77px] z-20  relative max-w-[1077px] mx-auto">
        <div className="flex gap-5 mx-2 xl:mx-0 md:gap-0 flex-col md:flex-row flex-wrap  bg-white justify-around my-5 md:my-0 rounded-none md:rounded-lg shadow-lg py-5 z-10 ">
          {bannerStates.map((item, index) => (
            <div className="flex flex-col items-center" key={item.title}>
              <h1 className="text-[#FF5349] mb-[22px] font-extrabold text-[43px]">
      ...
      <div  className="max-w-[1296px] mx-3 flex flex-col md:flex-row flex-wrap gap-6 justify-center lg:mx-auto mb-[120px]">
            {
                features.map((feature) => (
                    <div className={`md:w-[414px] p-[30px] cursor-pointer shadow-lg feature-box -z-20 relative rounded-lg ${feature.id == 1 && 'bg-[#FFEEED]'} ${feature.id == 2 && 'bg-[#E9EEF9]'} ${feature.id == 3 && 'bg-[#FFF5E6]'} `} key={feature.id}>
